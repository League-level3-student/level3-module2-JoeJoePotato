...
public class Algorithms {
	...
	public static List<String> sortWords(List<String> wurds) {
		...
		ArrayList<String> lesswurds = new ArrayList<String>();
		lesswurds.addAll(wurds);
	
		while(lesswurds.size()>0){
			String earliest="zzz";
			int indexfound=-1;
			for (int i = 0; i < lesswurds.size(); i++) {
				if (lesswurds.get(i).compareTo(earliest) < 0) {
					earliest = lesswurds.get(i);
					indexfound = i;
				}
			}
			orderedwurds.add(earliest);
			lesswurds.remove(indexfound);
		}
System.out.println(orderedwurds.toString());
		return orderedwurds;
	}
}
